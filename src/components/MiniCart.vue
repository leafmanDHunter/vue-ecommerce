          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                  <ul>
                    <li v-for="(item,index) in this.$store.state.cart" v-bind:key="index" class="media">
                      <img :src="item.productImage" width="80px" class="align-self-center mr-3" alt="">
                      <div class="media-body">
                        <h5 class="mt-0">{{item.productName}}

                          <span class='float-right delete-cursor' v-on:click="removeProductFromCart(item)">X</span>
                        </h5>
...
export default {
  ...
  methods:{
    ...
    removeProductFromCart(value) {
      this.$store.commit("removeProductFromCart", value);
    }
  }
};
</script>
